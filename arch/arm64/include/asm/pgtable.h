...
#define pte_cont_addr_end(addr, end)                                           \
	({                                                                     \
		unsigned long __boundary =                                     \
			((addr) + CONT_PTE_SIZE) & CONT_PTE_MASK;              \
...
#define pmd_cont_addr_end(addr, end)                                           \
	({                                                                     \
		unsigned long __boundary =                                     \
			((addr) + CONT_PMD_SIZE) & CONT_PMD_MASK;              \
		(__boundary - 1 < (end) - 1) ? __boundary : (end);             \
	})
...
static inline void set_pte(pte_t *ptep, pte_t pte)
{
	...
	if (!(old & new &PTE_RDONLY) && WARN_ON(pte_pfn(*ptep) != pte_pfn(pte)))
		goto pte_bad;
			      ...
			      pte_t *ptep, pte_t pte)
{
	 ...
	 */
	old_pte = READ_ONCE(*ptep);
	if (IS_ENABLED(CONFIG_DEBUG_VM) && pte_valid(old_pte) &&
	    pte_valid(pte) &&
		...
		VM_WARN_ONCE(
			...
			__func__, pte_val(old_pte), pte_val(pte));
		VM_WARN_ONCE(
			pte_write(old_pte) && !pte_dirty(pte),
			"%s: racy dirty state clearing: 0x%016llx -> 0x%016llx",
			__func__, pte_val(old_pte), pte_val(pte));
	}
...
#define set_pud_at(mm, addr, pudp, pud)                                        \
...
#define pte_offset_kernel(dir, addr)                                           \
	((pte_t *)__va(pte_offset_phys((dir), (addr))))
...
#define pud_offset_phys(dir, addr)                                             \
	(pgd_page_paddr(READ_ONCE(*(dir))) + pud_index(addr) * sizeof(pud_t))
...
/* use ONLY for statically allocated translation tables */
#define pud_offset_kimg(dir, addr)                                             \
	((pud_t *)__phys_to_kimg(pud_offset_phys((dir), (addr))))

#else

#define pgd_page_paddr(pgd)                                                    \
	({                                                                     \
		BUILD_BUG();                                                   \
		0;                                                             \
	})
...
#define update_mmu_cache_pmd(vma, address, pmd)                                \
	do {                                                                   \
	} while (0)
